	...
	case USBPD_SVDM_DISCOVER_IDENTITY:
		...
		if (num_vdos == 3) {
			...
			if (oplus_svid == OPLUS_SVID) {
				...
				if (handler) {
					usbpd_info(&pd->dev, "oplus_svid SVID: 0x%04x vdos1: %x vdos2 %x connect\n",
							handler->svid, vdos[1],
							vdos[2]);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	pd->use_external_boost = of_property_read_bool(parent->of_node,
			"otg-use_external_boost");
	if (pd->use_external_boost) {
		usbpd_info(&pd->dev, "wkcs: otg use external boost\n");
		pd->vbus_gpio = of_get_named_gpio(parent->of_node,
				"vbus-gpio", 0);
		...
		pd->otg_en_gpio = of_get_named_gpio(parent->of_node,
				"otg_en-gpio", 0);
